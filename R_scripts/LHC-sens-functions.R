...
get_data = function(path, file, start_year, end_year, growth_fac) {
	...
	NPP1 = DATA[min_row:max_row, 33]
	NEE1 = DATA[min_row:max_row, 34]
	BM1 = DATA[min_row:max_row, 35]
	RESP1 = DATA[min_row:max_row, 23]
	SOCA1 = DATA[min_row:max_row, 54]
	SOCS1 = DATA[min_row:max_row, 56]
	SOCP1 = DATA[min_row:max_row, 57]
	SOC1 = rowSums(DATA[min_row:max_row, 53:57])
	...
	SOCA = annual_average(SOCA1, years, start_year, len)
	SOCS = annual_average(SOCS1, years, start_year, len)
	SOCP = annual_average(SOCP1, years, start_year, len)
...
#	YIELD = BM * growth_fac * 2 / 100
	...
	summary = cbind(NPP, NEE, BM, RESP, SOCA, SOCS, SOCP, SOC11)
	return(summary)
}
...
# Get filenames for sensitivity analysis output including full filepath!
get.sensout.full = function (resdir) {
  sensnames = list.files(path = resdir, pattern = "sensitivity", full.names = TRUE)
  if (length(sensnames) > 6) stop("Too many 'sensitivity' files.")
  return(sensnames)
}
